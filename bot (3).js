...
const client = new Client({
  intents: [
    ...
    GatewayIntentBits.MessageContent, // requis pour lire le texte des messages (communiqués de presse)
  ]
});
...
client.once('ready', async () => {
  ...
  if (PRESS_AUTOMATION_READY) {
    console.log('📰 Communiqués de presse automatiques : actifs (salon', PRESS_CHANNEL_ID + ')\n');
  } else {
    console.log('📰 Communiqués de presse automatiques : DÉSACTIVÉS — variables manquantes :',
      [
        !PRESS_CHANNEL_ID && 'PRESS_CHANNEL_ID',
...
// ═══════════════════════════════════════════════════════════════════════════
// COMMUNIQUÉS DE PRESSE AUTOMATIQUES — Discord → site web
//
// Tout message posté dans le salon #communiqué-de-presse (voir PRESS_CHANNEL_ID
// ci-dessous) est reformulé en article par l'IA, puis publié dans le fichier
// articles-live.json à la racine du dépôt GitHub. Vercel redéploie le site
// automatiquement (~30s) et l'article apparaît sur montrealracingteam.ca :
// en vedette sur la page d'accueil, et dans « Toutes les actualités ».
//
// Variables d'environnement requises (à ajouter sur Railway) :
//   PRESS_CHANNEL_ID   — l'ID du salon Discord à surveiller
//                        (Discord : Paramètres > Avancé > Mode développeur,
//                         puis clic droit sur le salon > Copier l'ID du salon)
//   GITHUB_TOKEN       — jeton d'accès personnel GitHub avec droit d'écriture
//                        sur le dépôt (repo: montrealracingteam-MRT/V2)
//   GITHUB_REPO        — "montrealracingteam-MRT/V2" (propriétaire/dépôt)
//   GITHUB_BRANCH      — "main" (facultatif, défaut: main)
//   ANTHROPIC_API_KEY  — clé API Claude, pour reformuler le message en article
//   ARTICLE_MODEL      — modèle Claude à utiliser (facultatif,
//                        défaut: claude-sonnet-5)
//
// Si une de ces variables manque, le bot le signale une seule fois au
// démarrage et ignore simplement les messages du salon (aucun crash).
// ═══════════════════════════════════════════════════════════════════════════

const PRESS_CHANNEL_ID  = process.env.PRESS_CHANNEL_ID;
const GITHUB_TOKEN      = process.env.GITHUB_TOKEN;
const GITHUB_REPO       = process.env.GITHUB_REPO;   // ex: "montrealracingteam-MRT/V2"
const GITHUB_BRANCH     = process.env.GITHUB_BRANCH || 'main';
const ANTHROPIC_API_KEY = process.env.ANTHROPIC_API_KEY;
const ARTICLE_MODEL     = process.env.ARTICLE_MODEL || 'claude-sonnet-5';
const ARTICLES_FILE_PATH = 'articles-live.json';
const MAX_LIVE_ARTICLES  = 30; // garde les 30 derniers communiqués dans le fichier

const PRESS_AUTOMATION_READY = !!(PRESS_CHANNEL_ID && GITHUB_TOKEN && GITHUB_REPO && ANTHROPIC_API_KEY);
...
Réponds UNIQUEMENT avec un objet JSON valide, sans texte autour, sans balises markdown, au format exact suivant :
{
  ...
  const parsed = JSON.parse(text); // laisse volontairement remonter l'erreur si le JSON est invalide
  if (!parsed.title || !parsed.content) throw new Error('Réponse IA incomplète (title/content manquant)');
  return parsed;
}

// Récupère articles-live.json depuis GitHub (contenu + sha), ou [] s'il n'existe pas encore.
async function fetchLiveArticles() {
  const url = `https://api.github.com/repos/${GITHUB_REPO}/contents/${ARTICLES_FILE_PATH}?ref=${GITHUB_BRANCH}`;
...
// Ajoute le nouvel article en tête de liste et republie le fichier sur GitHub.
async function publishArticleToGithub(article, attempt = 1) {
  const { list, sha } = await fetchLiveArticles();
  const updated = [article, ...list].slice(0, MAX_LIVE_ARTICLES);
  const body = {
    message: `Communiqué de presse: ${article.title}`,
    content: Buffer.from(JSON.stringify(updated, null, 2), 'utf8').toString('base64'),
    branch: GITHUB_BRANCH,
  };
  if (sha) body.sha = sha;

  const url = `https://api.github.com/repos/${GITHUB_REPO}/contents/${ARTICLES_FILE_PATH}`;
  const res = await fetch(url, {
    method: 'PUT',
    headers: {
  ...
  // Conflit possible si deux communiqués arrivent en même temps (sha périmé) :
  // on relit le fichier et on retente une fois avant d'abandonner.
  if (res.status === 409 && attempt === 1) {
    return publishArticleToGithub(article, 2);
  }
  if (!res.ok) throw new Error(`GitHub PUT ${res.status}: ${await res.text()}`);
}

// ─── ÉCOUTE DU SALON "COMMUNIQUÉ DE PRESSE" ───────────────────────────────────
client.on('messageCreate', async message => {
  try {
    if (!PRESS_AUTOMATION_READY) return;
    if (message.author.bot) return;
    if (message.channelId !== PRESS_CHANNEL_ID) return;

    const rawText = (message.content || '').trim();
    const imgUrl = message.attachments.find(a => (a.contentType || '').startsWith('image/'))?.url || null;
    if (rawText.length < 8 && !imgUrl) return; // trop court pour être un vrai communiqué
    ...
    await publishArticleToGithub(article);

    await message.reactions.removeAll().catch(() => {});
    await message.react('✅').catch(() => {});
    await message.reply({
      content: `📰 Article publié sur le site : **${article.title}**\nEn ligne dans ~30s sur https://montrealracingteam.ca (section Actualités).`,
    }).catch(() => {});
  } catch (err) {
    console.error('❌ Erreur communiqué de presse:', err);
    await message.reactions.removeAll().catch(() => {});
    await message.react('❌').catch(() => {});
    await message.reply({
      content: `⚠️ Je n'ai pas pu publier ce communiqué automatiquement (${err.message}). Vérifie les variables d'environnement du bot ou réessaie.`,
    }).catch(() => {});
  }
});
